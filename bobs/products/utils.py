...
def un_authenticated_users_no_session(request: Type[HttpRequest]) -> Type[Order]:
    request.session.create()
    request.session["customer"] = request.session.session_key
    order, created = Order.objects.get_or_create(transaction_id=request.session["customer"],customer=None, complete=False)
    order.save()
    return order
